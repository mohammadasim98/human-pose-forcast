...
class PoseDecoder(nn.Module):
    ...
    def process_input(self, img_encoding: torch.Tensor, pos_encoding: torch.Tensor):
        ...
        total_encoding, attention_weights = self.dual_attention(
            img_encoding, 
            pos_encoding, 
            pos_encoding, 
...
class PoseDecoderV2(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.dual_local_attention = nn.MultiheadAttention(hidden_dim, num_heads, dropout=dropout, batch_first=batch_first)
        self.dual_global_attention = nn.MultiheadAttention(hidden_dim, num_heads, dropout=dropout, batch_first=batch_first)
        self.local_global_attention = nn.MultiheadAttention(hidden_dim, num_heads, dropout=dropout, batch_first=batch_first)
        self.decode_layer = nn.TransformerDecoderLayer(hidden_dim, num_heads, hidden_dim, dropout=dropout,
                                                       batch_first=batch_first, activation=activation())
