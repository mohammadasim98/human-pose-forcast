...
class HumanPosePredictorModelV2(nn.Module):
    ...
    def pose_encoding(self, poses, pose_mask: Union[torch.Tensor, None]=None):
        ...
        poses = rff.functional.positional_encoding(poses, sigma=1.0, m=32)
    ...
    def image_encoding(self, img_seq, mask):
        ...
        memory_temp, attention_weights = self.im_temporal_encoder(memory_local, mask=im_mask)
        # concatenate along sequence dimension (B, num_patches + history_window + 1, E)
        
        return memory_temp, attention_weights
    ...
    def forward(
        ...
        memory_img, image_attentions = self.image_encoding(
            img_seq=img_seq, 
            mask=img_mask
        )
        ...
        for i in range(0, future_window):
            ...
            memory_pose, pose_attentions = self.pose_temporal_encoder(encoded_poses, mask=None)
            dual_attention_weights = None

            memory, dual_attention_weights = self.dual_attention(
                query=memory_img, 
                key=memory_pose, 
                value=memory_pose, 
            ...
            memory, state = self.lstm(memory, state)
            
            result = self.decoder_layer(tgt=pose_encoding[:, -1, ...], memory=memory)
            ...
            pose_encoding = self.pose_encoding(
                ...
                pose_mask=future_pose_mask[:, i, ...] if future_pose_mask is not None and is_teacher_forcing else None
            )
