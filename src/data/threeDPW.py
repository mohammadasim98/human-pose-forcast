...
if __name__ == "__main__":
    ...
    """ Testing the dataset by displaying the scene and the corresponding poses.
    """
    
    tfrecord_path = os.path.join(".","data", "3DPWPreprocessed", "tfrecords_combined", "train.tfrecord")

    dataset = ThreeDPWTFRecordDataset(tfrecord_path, history_window=15, future_window=15)
    print(f"Length of dataset: {len(dataset)}")
    
    loader = dataset.get_loader()

    for i, (history, future) in enumerate(loader):
        img = deepcopy(history[0][0][0].numpy())
